...
12) skills categorisation and deduplication: tools_and_platforms are specific named software, tools, SaaS products, programming languages, cloud services; methodologies are processes, frameworks, and ways of working; domain_expertise is business and functional domain knowledge; soft_skills are interpersonal and leadership capabilities; top_skills must contain exactly the 5 skills that best represent this candidate from across all categories. SKILLS EXTRACTION ANTI-HALLUCINATION RULE: Candidate skills fields must contain ONLY skills, tools, technologies, methodologies, domain terms, and soft skills explicitly named or clearly stated in the resume text. Do NOT infer, assume, or transfer skills from the job description into the candidate's skills fields. If the JD requires a skill but the resume does not mention it, that skill must NOT appear in skills or top_skills; place it only in fit_assessment.missing_requirements or fit_assessment.risks_or_gaps when relevant. When in doubt, omit the skill. DEDUPLICATION: before including any skill, check if a semantically equivalent skill is already in the list and keep only one normalized form (e.g., "Agile (Scrum)" over "Agile Scrum", "Jira" over "JIRA").
