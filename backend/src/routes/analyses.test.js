...
test('GET /analyses returns authenticated user scoped items with frontend fields', async (t) => {
  ...
  assert.equal(payload.items[0].summary.totalResumes, 3)
  assert.equal(payload.items[0].summary.successCount, 1)
  assert.equal(payload.items[0].summary.partialCount, 1)
  assert.equal(payload.items[0].summary.failedCount, 1)
...
test('GET /analyses emits stable status summaries for complete/partial/processing/failed states', async (t) => {
  ...
  assert.equal(payload.items[1].summary.totalResumes, 3)
  assert.equal(payload.items[1].summary.successCount, 2)
  assert.equal(payload.items[1].summary.partialCount, 0)
  assert.equal(payload.items[1].summary.failedCount, 1)
...
test('GET /analyses/:id returns owner-only detail payload', async (t) => {
  ...
  assert.equal(payload.summary.totalResumes, 1)
  assert.equal(payload.summary.successCount, 1)
  assert.equal(payload.summary.partialCount, 0)
...
test('GET /analyses/:id mixed batch completes with per-resume parse outcomes and accurate counters', async (t) => {
  process.env.JWT_SECRET = 'test-secret'
  t.mock.method(parseQueue, 'getJob', async () => null)
  t.mock.method(pool, 'query', async (sql) => {
    if (sql.includes('FROM analyses a')) {
      return { rows: [{ id: 55, user_id: 9, status: 'processing', created_at: '2026-05-01T00:00:00.000Z', completed_at: null, error_summary: null, job_description_id: null, job_description_title: null }] }
    }
    if (sql.includes('FROM analysis_items ai')) {
      return { rows: [
  ...
  const server = app.listen(0)
  const port = server.address().port
  const response = await fetch(`http://127.0.0.1:${port}/analyses/55`, { headers: authHeader(9) })
  const payload = await response.json()
  server.close()
