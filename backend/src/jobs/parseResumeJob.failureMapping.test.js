...
import { normalizeFailureCategory } from '../contracts/parseResultEnums.js'
...
test('structured local parse failure category does not collapse to unknown', () => {
  const category = normalizeFailureCategory('parse_failed::ai_output_validation_failed', { fallback: 'unknown' })
  assert.equal(category, 'ai_output_validation_failed')
})
