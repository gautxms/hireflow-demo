      ...
      {alerts?.length > 0 && (
        <div className="mt-4 grid gap-2">
          {alerts.map((alert) => (
            <div
              key={alert.message}
              className={`rounded-md border px-3 py-2 text-sm ${alert.severity === 'critical' ? 'admin-inline-alert admin-inline-alert--error' : 'admin-inline-alert admin-inline-alert--warning'}`}
            >
