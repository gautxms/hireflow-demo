...
export function FormRow({ label, htmlFor, hint, children, className = '' }) {
  return (
    <div className={cx('admin-form-row', className)}>
      {label ? <label htmlFor={htmlFor} className="admin-form-row__label">{label}</label> : null}
      {children}
      {hint ? <p className="admin-form-row__hint">{hint}</p> : null}
    </div>
  )
}
...
export function Alert({ tone = 'info', children, className = '', role }) {
  ...
  return (
    <div className={cx('admin-inline-alert', `admin-inline-alert--${tone}`, className)} role={resolvedRole}>
      {children}
    </div>
  )
}
