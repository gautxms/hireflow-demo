    ...
    <main className="admin-page">
      ...
      <section className="ui-card p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          <label className="text-sm text-admin-body">Start date
          ...
          <label className="text-sm text-admin-body">End date
      ...
      {loading ? <div className="ui-card p-4 text-admin-body">Loading analytics…</div> : null}
      {error ? <StateAlert state={error} onRetry={refresh} /> : null}
      {!error && analytics && dataMode?.limited ? (
        <section className="ui-card border border-admin-warning bg-admin-warning-subtle p-4">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <h2 className="text-sm font-semibold uppercase tracking-wide text-admin-warning-strong">Limited data mode</h2>
              <p className="mt-1 text-sm text-admin-warning">
                Some analytics sections are unavailable. Zeroes in unavailable widgets are placeholders, not real counts.
              </p>
              {dataMode?.diagnostics ? <p className="mt-2 text-xs text-admin-warning">Diagnostics: {dataMode.diagnostics}</p> : null}
      ...
      {analytics && !error ? (
        <>
          ...
          <div className="grid gap-4 xl:grid-cols-3">
            ...
            <section className="ui-card p-4">
              <h2 className="text-lg font-medium text-admin-strong">Plan Breakdown</h2>
              ...
              <div className="mt-4 space-y-3">
                {(analytics.planBreakdown || []).map((plan) => (
                  <div key={plan.plan}>
                    ...
                    <div className="h-4 rounded bg-admin-subtle">
            ...
            <section className="ui-card p-4">
              <h2 className="text-lg font-medium text-admin-strong">Parsing Stats</h2>
          ...
          <section className="ui-card p-4">
            <h2 className="text-lg font-medium text-admin-strong">AI Token Usage</h2>
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {(analytics.tokenUsageTrend || []).slice(-14).map((row) => (
                    <tr key={String(row.day)} className="border-t border-admin">
          ...
          <section className="ui-card p-4">
            <h2 className="text-lg font-medium text-admin-strong">Per-upload Token Usage (latest 25)</h2>
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {(analytics.tokenUsageUploads || []).slice(0, 25).map((row, index) => (
                    <tr key={`${row.resumeId || 'resume'}-${row.createdAt || index}`} className="border-t border-admin">
                  ...
                  {(!analytics.tokenUsageUploads || analytics.tokenUsageUploads.length === 0) ? (
                    <tr className="border-t border-admin">
                      <td className="py-3 text-admin-muted" colSpan={8}>No token usage records in this range.</td>
                    </tr>
                  ) : null}
                </tbody>
              </table>
          ...
          <section className="ui-card p-4">
            <h2 className="text-lg font-medium text-admin-strong">Retention Cohorts Heatmap</h2>
            ...
            <div className="mt-4 overflow-auto">
              <div className="grid min-w-[720px] grid-cols-[160px_repeat(10,minmax(0,1fr))] gap-2 text-xs">
                <div className="font-medium text-admin-muted">Cohort</div>
                {Array.from({ length: 10 }, (_, offset) => <div key={offset} className="text-center font-medium text-admin-muted">W+{offset}</div>)}
                {[...new Set((analytics.retentionCohorts || []).map((row) => row.cohort_week))].map((cohort) => {
                  ...
                  return (
                    <div key={cohort} className="contents">
                      <div key={`${cohort}-label`} className="rounded bg-admin-subtle p-2 text-admin-body">{new Date(cohort).toLocaleDateString()}</div>
                      {Array.from({ length: 10 }, (_, offset) => {
                        ...
                        return (
                          <div
                            ...
                            className="rounded p-2 text-center font-medium text-admin-body"
          ...
          <section className="ui-card p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h2 className="text-lg font-medium text-admin-strong">Weekly UX Health (Founder Review)</h2>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              <div className="rounded border border-admin p-3 text-sm">
                <p className="text-admin-muted">2FA completion rate</p>
                <p className="text-xl font-semibold text-admin-strong">{pct(analytics.uxWeeklyReport?.twoFactorCompletionRate || 0)}</p>
                <p className="text-xs text-admin-muted">{analytics.uxWeeklyReport?.twoFactorCompleted || 0}/{analytics.uxWeeklyReport?.twoFactorStarted || 0} completed</p>
              </div>
              <div className="rounded border border-admin p-3 text-sm">
                <p className="text-admin-muted">Admin page feedback</p>
                <p className="text-xl font-semibold text-admin-strong">{analytics.uxWeeklyReport?.adminFeedbackSummary?.total || 0}</p>
                <p className="text-xs text-admin-muted">{analytics.uxWeeklyReport?.adminFeedbackSummary?.notUseful || 0} marked not useful</p>
              </div>
              <div className="rounded border border-admin p-3 text-sm">
                <p className="text-admin-muted">Tracked blockers</p>
                <p className="text-xl font-semibold text-admin-strong">{analytics.uxBlockers?.length || 0}</p>
                <p className="text-xs text-admin-muted">By event frequency in selected window</p>
              </div>
            </div>

            <div className="mt-4 grid gap-4 xl:grid-cols-2">
              <div>
                <h3 className="text-sm font-semibold text-admin-title">Top blockers by frequency</h3>
                <ul className="mt-2 space-y-2 text-sm">
                  {(analytics.uxWeeklyReport?.topBlockers || []).map((blocker, index) => (
                    <li key={`${blocker.event_type}-${index}`} className="rounded border border-admin p-2">
                      <p className="font-medium text-admin-strong">#{index + 1} {blocker.event_type}</p>
                      <p className="text-admin-body">Route: {blocker.route}</p>
                      <p className="text-admin-body">Frequency: {blocker.frequency}</p>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div>
                <h3 className="text-sm font-semibold text-admin-title">Data-backed next sprint priorities</h3>
                <ol className="mt-2 space-y-2 text-sm">
                  {(analytics.uxWeeklyReport?.nextSprintPriorities || []).map((item) => (
                    <li key={`${item.rank}-${item.blocker}`} className="rounded border border-admin-info bg-admin-info-subtle p-2">
                      <p className="font-medium text-admin-strong">{item.rank}. {item.blocker} ({item.frequency})</p>
                      <p className="text-admin-body">{item.recommendation}</p>
                    </li>
                  ))}
                </ol>
              </div>
