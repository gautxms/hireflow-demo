...
test('candidate tag filters live inside the filter popover and count as active filters', () => {
  ...
  assert.match(candidateFiltersSource, /<div className="fp-primary-grid">/)
  assert.match(candidateFiltersSource, /<div className="fp-label">Tags<\/div>/)
  assert.match(candidateFiltersSource, /const \[tagSearch, setTagSearch\] = useState\(''\)/)
  assert.match(candidateFiltersSource, /placeholder="Search tags\.\.\."/)
  assert.match(candidateFiltersSource, /No tags available/)
  assert.match(candidateFiltersSource, /aria-pressed=\{selected\}/)
  assert.match(candidateResultsStyles, /\.fp-primary-grid\s*\{[^}]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\)/s)
  assert.match(candidateResultsStyles, /@media \(max-width: 840px\)[\s\S]*\.fp-primary-grid\s*\{[^}]*grid-template-columns:\s*1fr/)
  assert.match(candidateFiltersSource, /onTagsFilter\?\.\(\[\]\)/)
})
...
test('candidate cards limit tag density without reserving blank tag space', () => {
  ...
  assert.match(candidateResultsStyles, /\.rc-tag-more\s*\{[^}]*flex:\s*0 0 auto/s)
})
