  ...
  return (
    <section className="shortlist-manager" aria-label="Shortlists page">
      ...
      <section className="shortlist-manager__list-card" aria-label="Paginated shortlist list and cards">
        ...
        {selectedShortlist && !loadingDetails && allCandidates.length > 0 && filteredCandidates.length === 0 ? <div className="shortlist-manager__empty"><p>No candidates match your current filters.</p><button type="button" className="shortlist-manager__button shortlist-manager__button--neutral" onClick={() => { setCurrentPage(1); setQuery(''); setFilters({ decisionStatus: 'all', rating: 'all', analysisSource: 'all' }) }}>Clear filters</button></div> : null}
      </section>
    </section>
  )
}
