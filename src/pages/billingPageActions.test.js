...
test('BillingPage keeps recovery state and polling until the adjustment is terminal', () => {
  ...
  assert.match(source, /!nextSubscription\?\.recoveryAdjustmentStatus[\s\S]*setRecoveryPending\(false\)[\s\S]*replaceState/)
  assert.doesNotMatch(source, /recoveryAdjustmentStatus: 'pending'/)
...
test('BillingPage offers state-specific keep, payment update, and subscribe-again actions', () => {
  ...
  assert.match(source, /event\.preventDefault\(\)[\s\S]*onNavigate\('\/pricing\?reason=subscribe_again'\)/)
