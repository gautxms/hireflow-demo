...
| BGX-004 | Cross-app token migration backlog (`src/**`) | Temporarily baseline pre-existing style-token findings while Phase 6–9 targeted pages are remediated | Repository currently contains historical hardcoded/legacy token usages outside active phase scope; broad cleanup requires multi-route migration sequence | Design Systems + Frontend | 2026-04-20 | **2026-05-10** | Renew only if remaining backlog is actively being reduced per week and a scoped migration PR is linked | Follow-up migration tracker: "style-token backlog burn-down (post Phase 9)" | Active |
